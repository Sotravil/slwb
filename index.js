...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const apiKey = setting.apiKey // contact me on whatsapp wa.me/19728806294
			...
			mess = {
				...
				only: {
					...
					Badmin: '❌ Este comando solo se puede usar cuando el bot es un administrador! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = [`${setting.19728806294}@s.whatsapp.net`] // replace this with your number
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Botstickers'; if (!author) author = 'Sotravil';	
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('MishiCrawn', authorname)} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, '¿Dónde está el nombre de usuario?', text, {quoted: mek})
